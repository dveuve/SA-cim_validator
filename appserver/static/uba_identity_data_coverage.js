...
require([
    ...
    var appName = utils.getCurrentApp()
        ...
        launchCheck("department", "error", "Validate that a department is defined for each record", '| stats count as total count(eval(isnull(department) OR department="")) as withoutdept values(eval(if(isnull(department) OR department="", sAMAccountName, null))) as userwithoutdept | eval status=if(withoutdept=0, "ok", "bad"), description=if(withoutdept=0, "Check Passed", "Not all user accounts have Departments. Out of " . total . " accounts, " . withoutdept . " do not have an department field defined. The first few accounts without are: " . mvjoin(mvindex(userwithoutdept, 1,10), ", ")) ')
        ...
        launchCheck("OrphanedAdminAccounts","warn","Validate every admin account has resolved to at least 1 other account (Hopefully a non privileged account)", '| stats dc(hrAccountType) AS num_account_types dc(sAMAccountName) AS num_accounts values(hrAccountType) AS hrAccountType values(sAMAccountName) AS sAMAccountName values(distinguishedName) values(givenName) values(sn) AS sn BY employeeid | where hrAccountType="Admin" AND num_accounts=1 | stats count as total values(sAMAccountName) AS sAMAccountName | eval status=if(total=0,"ok","bad"),description=if(total=0,"Check Passed","There are admin accounts we cannot resolve to any other non privileged account. The first few are: " . mvjoin(mvindex(sAMAccountName, 1,10), ", ") . ". You may wish to investigate.")')
